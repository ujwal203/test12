...
import { FiSearch, FiMapPin, FiBriefcase, FiAward, FiDollarSign, FiClock, FiFilter, FiXCircle, FiGlobe } from 'react-icons/fi'; // Added FiGlobe for company icon
...
export default function FindJobPage() {
  ...
  const [showMobileFilters, setShowMobileFilters] = useState(false); // State for mobile filter sidebar
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    fetchJobs(); // Fetch immediately on submit
    setShowMobileFilters(false); // Close filters on mobile after search
  };
  ...
  const handleClearFilters = () => {
    ...
    setExperienceLevel('');
    setCompanyName('');
    router.push('/jobs/find'); // Clear URL params
  ...
  return (
    // Removed pt-16 from here, added mt-16 to the inner max-w-7xl div
    <div className="min-h-[calc(100vh-64px)] bg-gray-50 py-8 sm:py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16"> {/* Added mt-16 here to push content below navbar */}
          ...
          <aside className={`fixed inset-y-0 left-0 transform ${showMobileFilters ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 lg:col-span-1 bg-white p-6 rounded-xl shadow-2xl lg:shadow-xl transition-transform duration-300 ease-in-out z-40 w-80 lg:w-auto overflow-y-auto top-16`}>
            ...
            <form onSubmit={handleSearchSubmit} className="space-y-6">
              ...
              <div className="flex flex-col space-y-3 pt-4">
                <button
                  type="submit"
                  className="w-full bg-gradient-to-r from-[#741ee3] to-[#9a4dff] text-white font-bold py-3 px-4 rounded-lg shadow-md hover:from-[#5a16b5] hover:to-[#741ee3] transition-all focus:outline-none focus:ring-2 focus:ring-[#741ee3] focus:ring-offset-2"
                >
                  Apply Filters
                </button>
                <button
                  ...
                  className="w-full bg-gray-200 text-gray-800 font-bold py-3 px-4 rounded-lg shadow-md hover:bg-gray-300 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2"
                >
                  Clear Filters
                </button>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-2xl font-bold text-gray-900">
                    {jobPosts.length} {jobPosts.length === 1 ? 'Job' : 'Jobs'} Found
                  </h2>
                  {/* Sorting dropdown could be added here */}
                </div>

                {/* Changed grid to always be a single column for job cards, occupying full width */}
                <div className="grid grid-cols-1 gap-6">
                  {jobPosts.map((job) => (
                    <div key={(job._id as any).toString()} className="group relative bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300 border border-gray-100 w-full"> {/* Added w-full */}
                      {/* Featured Ribbon (if applicable) */}
                      {job.isFeatured && (
                        <div className="absolute top-0 right-0 bg-gradient-to-br from-[#741ee3] to-[#9a4dff] text-white text-xs font-bold px-3 py-1 rounded-bl-lg z-10">
                          Featured
                        </div>
                      )}
                      ...
                      <div className="p-6">
                        <div className="flex items-start mb-4">
                          <div className="flex-shrink-0 h-14 w-14 rounded-full bg-gradient-to-br from-purple-100 to-indigo-100 flex items-center justify-center text-[#741ee3] shadow-md">
                            {job.company?.logoUrl ? (
                              <img src={job.company.logoUrl} alt={job.company.name} className="h-10 w-10 object-contain rounded-full" />
                            ) : (
                              <FiBriefcase className="h-7 w-7" />
                            )}
                          </div>
                          <div className="ml-4 flex-1">
                            <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#741ee3] transition-colors leading-tight">
                              {job.title}
                            </h3>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4 text-sm text-gray-600 mb-4">
                          <div className="flex items-center">
                            <FiMapPin className="mr-2 h-4 w-4 text-gray-500 flex-shrink-0" />
                            {job.location}
                          </div>
                          <div className="flex items-center">
                            <FiClock className="mr-2 h-4 w-4 text-gray-500 flex-shrink-0" />
                            {job.jobType}
                          </div>
                          <div className="flex items-center">
                            <FiAward className="mr-2 h-4 w-4 text-gray-500 flex-shrink-0" />
                            {job.experienceLevel}
                          </div>
                          {job.salaryRange && (
                            <div className="flex items-center">
                              <FiDollarSign className="mr-2 h-4 w-4 text-gray-500 flex-shrink-0" />
                              {job.salaryRange}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2 mb-4">
                          {job.skillsRequired.slice(0, 5).map((skill, index) => (
                            <span
                              key={index}
                              className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-[#741ee3] shadow-sm"
                            >
                              {skill}
                            </span>
                          ))}
                        ...
                        <p className="mt-4 text-sm text-gray-700 line-clamp-3 mb-6">
                          {job.description}
                        </p>
                        ...
                        <div className="flex justify-between items-center border-t border-gray-100 pt-4">
                          ...
                          <Link
                            href={`/jobs/${(job._id as any).toString()}`}
                            className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-gradient-to-r from-[#741ee3] to-[#9a4dff] hover:from-[#5a16b5] hover:to-[#741ee3] transition-all"
                          >
                            View Details
