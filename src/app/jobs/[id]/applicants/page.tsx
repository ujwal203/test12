...
export default function JobApplicantsPage({ params }: NextJsPageProps) {
  ...
  }, [session, status, router, jobId]); // Add jobId to dependencies
  ...
  // Show loading state if session is loading, or if jobId is not yet resolved, or if data is loading
  if (status === 'loading' || loading || jobId === null) {
    return (
