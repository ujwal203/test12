...
// Renamed the function to LoginClient
export default function LoginClient() {
  ...
  useEffect(() => {
    ...
    if (urlError === 'ReferralExpired') {
      ...
      // Catch-all for other NextAuth errors from the URL
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      if (result.error === 'CredentialsSignin') {
        ...
        // Use the error message directly from authOptions if it's a custom one
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <div className="relative">
                  <input type="email" id="email" className="block w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#741EE3] focus:border-[#741EE3] transition-all placeholder-gray-400" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="you@example.com" />
              ...
              {email === 'admin@admin.com' ? (
                <div>
                  ...
                  <div className="relative">
                    <input type="password" id="password" className="block w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#741EE3] focus:border-[#741EE3] transition-all placeholder-gray-400" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="••••••••" />
                    <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
              ...
              <div>
                <button type="submit" disabled={loading} className={`w-full flex justify-center items-center py-3 px-4 rounded-lg transition-all ${loading ? 'bg-[#9D5CFF]' : 'bg-gradient-to-r from-[#741EE3] to-[#9D5CFF] hover:from-[#5A17B5] hover:to-[#741EE3]'} text-white font-medium shadow-md hover:shadow-lg`}>
                  {loading ? (
                    <>
